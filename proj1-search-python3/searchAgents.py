...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.cornerState = startingGameState
    ...
    def getStartState(self):
        ...
        startState = (self.startingPosition, ()
                      )  # Make one more state for corner state
        return startState
    ...
    def isGoalState(self, state):
        ...
        """
        sortTuple = tuple(
            sorted(state[1]))  # sort a list of corner state to compare with self.corner
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not nextHitsWall:
                nextState = (nextx, nexty)
                # Get visited corner state's information from pacman state
                nextCorner = set(state[1]).copy()
                # print(nextCorner)
                if nextState in self.corners:       # If pacman goes to the corner
                    # if next state(corner) is already stored in corner state's information; To prevent add duplicate corner states, if not it would be messed up
                    if nextState not in nextCorner:
                        # Update corner state informations
...
def cornersHeuristic(state, problem):
    ...
    # Get visited corner state information from pacman
    set1 = set(state[1]).copy()
    set2 = set(corners).copy()  # Original corner state information
    # print(set1, set2)
    unvisited = list(set2.symmetric_difference(set1))   # Get unvisited state
    # print(unvisited)

    maxHeu = 0                  # heuristic value
    distances = []

    if problem.isGoalState(state):
        return 0
    for u in unvisited:
        distances.append(mazeDistance(u, state[0], problem.cornerState))
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        distances.append(mazeDistance(
            food, position, problem.startingGameState))
    maxHeu = max(distances)
    return maxHeu
